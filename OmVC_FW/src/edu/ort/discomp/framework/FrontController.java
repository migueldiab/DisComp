...
public class FrontController extends HttpServlet {
  ...
  private void processFrontControllerRequest(final HttpServletRequest request, final HttpServletResponse response)
    ...
    boolean dispatched = Boolean.FALSE;
      ...
      if (command.getPath().equals(commandCalled)) {
        if (command.getUrl() != null) {
          try {
            ...
            new CommandFactory().forwardByActionMap(this, CommandFactory.ERROR, request, response);
          }
    ...
    if (!dispatched) {
      try {
        int end = commandCalled.indexOf(".cmd");
        final String webActionClass = "actions."+commandCalled.substring(1, end);
        new CommandFactory().forwardByReflection(this, webActionClass, request, response);
      } catch (Exception e) {
        new CommandFactory().forwardByActionMap(this, CommandFactory.ERROR, request, response);
      }
  ...
  private void forwardByURL(String url, HttpServletRequest request, HttpServletResponse response)
    ...
    request.setAttribute(CommandFactory.VIEW, url);
  ...
  private void forwardByType(Command command, HttpServletRequest request, HttpServletResponse response)
    ...
    final String responseForward = new CommandFactory().forwardByReflection(this, command.getType(), request, response);
